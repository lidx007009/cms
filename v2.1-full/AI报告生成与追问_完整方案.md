...
# 第七部分 · 取数与计算时机 FAQ（实时编译 / 预聚合 / 预计算宽表）

> 这一节回答"指标到底是怎么算出来的、什么时候算、要不要预计算成宽表"，帮团队对齐认知、避免选型踩坑。

## 7.1 语义层（Cube/dbt/Malloy）到底"实时"到什么程度
1. **实时生成 SQL = 是**：你给语义层一个"语义查询"(指标+维度+过滤+时间)，它在查询那一刻**动态编译出 SQL**，不是事先写死。
2. **谁在算 = 底层数仓/OLAP，不是语义层**：语义层负责"编译 SQL → 下推(push down)到数仓(ClickHouse/StarRocks/BigQuery/Snowflake 等) → 拿结果回传"，本身不存数据、不做大规模计算。
3. **是否每次都全量实时算 = 看有没有配预聚合/缓存**：

| 模式 | 行为 | 数据新鲜度 | 适用 |
|---|---|---|---|
| 不配预聚合 | 每次查询真·实时扫底表计算 | 绝对实时 | 数据量小 / 临时探索 |
| 配 pre-aggregation(预聚合 rollup) | 命中物化聚合层，不扫全表 | =预聚合刷新周期(近实时) | 数据量大 / 高频(推荐) |
| 结果缓存 | 相同查询直接返回 | 缓存 TTL 内 | 重复查询 |

> ratio 类指标(如**毛利率 = SUM(amount-cost)/SUM(amount)**)由语义层保证"**先聚合分子分母再相除**"，不会错算成"行级比率求平均"。

## 7.2 本方案里：报告 vs 追问，取数时机不同
- **生成报告(场景 A)**：值在**报告生成那一刻算一次**，然后**冻结进版本快照** `frozen_values`；之后查看/追问引用都读冻结值，不重算 —— 为了可复现、可对账、可审计。
- **报告内追问(场景 B)**：**实时查询**(语义层编译 SQL + 预聚合/缓存 + 下推数仓)，这才是典型的"实时生成 SQL、实时计算"。

```
报告毛利率 24.1%  ──(报告生成时算一次)──►  冻进快照，永远是这个值(对账用)
追问"按品类看毛利率" ──(实时编译SQL + 预聚合/缓存 + 下推数仓)──►  当下结果
```

## 7.3 ⚠️ 哪些适合"预计算成宽表"，哪些不适合
**重要取舍：预计算宽表是为特定查询形态"定制"的，性能/稳定好，但定制化强、灵活性差**——新增一个维度或改口径就要改 ETL、回刷历史；维度组合一多，宽表行列会爆炸；还容易和语义层形成"双口径"漂移。因此遵循一条原则：

> **能用语义层(+预聚合)就别手搓宽表；只有"固定、高频、重计算、要冻结"的场景才下沉到宽表。**

| | 适合"预计算成宽表" | 不适合宽表，应走"实时语义层" |
|---|---|---|
| 典型场景 | 报告固定槽位指标、首页大盘看板、财务/合规上报 | 报告内无尽追问、任意下钻、临时探索 |
| 查询形态 | **固定、可枚举**(就那几个指标×几个维度) | **不可预测、长尾**(维度组合任意) |
| 维度基数 | 低、组合少 | 高、组合爆炸(宽表会笛卡尔积膨胀) |
| 计算复杂度 | 极重、跨多表多步、实时算不动 | 中等、单表/可下推 |
| 一致性要求 | 极高、需冻结对账 | 即时、容忍近实时 |
| 变更频率 | 口径稳定、很少改 | 口径还在迭代、经常加维度 |

**判断口诀**：
- **固定 + 高频 + 重 + 要冻结 → 预计算宽表**（如：月度报告的核心指标 → 预计算 + 快照冻结）。
- **多变 + 长尾 + 要灵活 → 实时语义层**（如：追问的任意维度下钻）。
- **高频但维度组合有限 → 用语义层的 pre-aggregation(rollup)**，这是"受管理的预计算"：比手搓宽表灵活(语义层自动判断是否命中 rollup、未命中自动回退底表)，又比纯实时快。**优先选它，而不是自己堆宽表。**

**落地建议**：
1. 报告(场景 A)用"**预计算/预聚合 + 快照冻结**"，稳定可对账；
2. 追问(场景 B)用"**实时语义层 + pre-aggregation + 缓存**"，保留灵活；
3. 即使要做宽表，也**让宽表由语义层定义/对齐口径**（`metrics.yaml` 的 `impl` 指向该宽表），避免"宽表一套口径、语义层另一套口径"的漂移。

---

# 第八部分 · 平台功能设计（开发态/运行态 + Agent/Skill 编排）

> 落地不只是写代码，更要设计成"平台"，让**业务方后期能自助开发报告 / 查看报告 / 追问**。详细设计见 `examples/18-platform/buildtime_vs_runtime.md`，这里给要点。

## 8.1 总原则：开发态产出制品，运行态消费制品
**开发态(设计/编排/认证/评测/构建索引) 产出带版本的"制品" → 经【注册中心 + 评测门禁】发布 → 运行态(取数/生成/对话/拦截/调度) 只消费已发布版本。** 制品包括：metrics 定义、报告模板+映射契约、章节 Prompt、Skill 定义、few-shot、巡检规则、护栏策略、评测集。全部可版本化、可灰度、可回滚。

## 8.2 哪些是开发态、哪些是运行态（速查）
- **开发态**：语义建模、报告设计器(拖指标→自动生成契约)、Prompt 工作台、Skill 注册中心、评测集编写、RAG 索引构建、巡检规则/护栏策略配置、few-shot 治理。
- **运行态**：报告生成服务、报告查看服务、追问 Agent、Skill/MCP 执行、语义层查询引擎、主动洞察调度、护栏中间件、可观测/缓存、反馈采集。
- **两态都有**：Skill(注册=开发态，执行=运行态)、RAG(建索引=开发态，召回=运行态)、反馈飞轮(采集=运行态，回灌=离线)。
> 完整归属表见平台文档第 1 节。
...
## 8.4 更好的设计（5 条增强）
1. **以"注册中心 + 制品"为唯一契约**：把方案从一堆脚本升级为平台。
2. **环境分离 + 灰度/影子发布 + 一键回滚**(dev/staging/prod)。
3. **低代码报告设计器 + 只能选 certified 指标** → 业务自助开发报告，口径受控、契约自动生成、CI 校验。
4. **能力统一注册(Capability Registry)**：报告/追问/预测/洞察都是"能力"，加新能力=注册新 Skill，不改 Agent。
5. **评测门禁左移到开发态**：报告/Prompt/Skill 发布前必须过评测(blocker 100%)。

> 平台落地分期(P1 报告生成查看 → P2 追问 Agent+Skill+护栏 → P3 低代码设计器+主动洞察+飞轮)见平台文档第 6 节。
